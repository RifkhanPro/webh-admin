...
function AddTopicPost() {
  ...
  const [names, setNames] = useState()
      ...
      useEffect(() => {
        const sendRequest = async () => {
          try {
              const response = await fetch('http://44.202.187.100:8070/topic/topicNames', {method:"POST", headers : {"Content-Type":"application/json"}, body :JSON.stringify({
                category
              })
            })
  ...
  const submitHandler =  async (e) => {
    ...
    try {
			const response = await fetch('http://44.202.187.100:8070/topicPost', {method:"POST", headers : {"Content-Type":"application/json"}, body :JSON.stringify({
          category,
          name,
					desc,
          image
